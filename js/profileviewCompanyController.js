...
app.controller('profileviewCompanyController', function ($scope,$mdDialog,$state,$timeout,$http,getfitService,$localStorage,$location,fetchrecordsCMSService,updateProfileService,removeImageService,getMatercmsService,getProfileService,checkjobService) { 
		 ...
		 if($localStorage.to_view_user_id != ''){

			 $localStorage.ses_userdata.users_id = $localStorage.to_view_user_id;
			 $scope.to_view_user_id = $localStorage.to_view_user_id;
			 $scope.userInfo_type = 1;
	...
	var fetch_skilldatalist = function (data) {
	    ...
	    fetchrecordsCMSService.fetchrecordsCMS('','getlocationdata',$localStorage.ses_userdata.users_id).then(get_datalist, errorDetails); 
